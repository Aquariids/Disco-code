...
Работает это с  помощью некого `API` которое работает на сервере и принимает запросы на данные и отправляет данные обратно в качестве ответа. <span className = 'link_js'> [Подробнее тут](/web-request-response) </span>.
### Что такое api
