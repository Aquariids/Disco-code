...
1. Это использование стандарнтых заводских настроек.
   >Когда мы запускаем проект без `webpack.config.js` для работы вебпака у нас должен быть главный файл который принимает
   все импорты  и в таком режиме работы он обязательно должен называться `index.js` и находиться в папке `src` -  `src/index.js`.
