...
Вспомним опять концептуальное представление `lexical environment` из урока про лексическое окружение. Смотрим на свойство `outer`.
...
a();
```
Теперь представим это в небольшом псевдо коде.
...
Это и есть `scope chain`. Это все концепция  `Lexical scoping(scope)`.
...
<div className='container-img'> <span className="img"> ![call stack](/images/Content/Js/under-the-hood-js/scope/call_stack.png) </span></div>

Так же указано окружение переменных(`variable environment`) для каждого контекста выполнения.

### Смотрим на scope chain

Теперь на основе `variable environment  ` мы построим `scope chain`.
