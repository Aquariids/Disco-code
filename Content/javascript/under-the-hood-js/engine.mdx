...
* (call stack)Стек вызовов - это место <b>где</b> код выполняется. Выполняется он при использовании <b>контекста выполнения</b>. Так же в контекста выполнения хранятся все наши примитивные значения и ссылки на объекты и функции. 
...
Об этих компонентах движка мы подробнее поговорим в следующем уроке.
...
такое медленное выполнение уже никуда не идет. Сейчас `js` все еще интерпретируемый, но теперь JIT (Just-In-Time) компиляция используется для оптимизации работы.
...
Такой процесс оптимизации может происходить несколько раз, после каждой оптимизации, оптимизируемый код заменяет старый код. Такая оптимизация позволяет движкам быть очень быстрыми.
