...
такое медленное выполнение уже никуда не идет. Поэтому сейчас `js` все еще интерпретируемый, но теперь JIT (Just-In-Time) компиляция используется для оптимизации работы.
