...
<div className = 'tag_js'>[Lexical environment(Лексическое окружение или среда) & heap(куча):](#lexical-environmentлексическое-окружение-или-среда--heapкуча)</div>
...
## Lexical environment(Лексическое окружение или среда) & heap(куча)
...
### Пример с var & if & let. Variable environment.
...
Лексическое окружение было только у глобального контекста и у контекста функций. Понятие `Lexical Environmentа` на тот момент уже существовало, но именно с появлением `let` и `const` ситуация изменилась, они расширили использование лексического окружения на блочный уровень и лексическое окружение появлись у любых блоков кода, но при этом `var` как работали так и работают.
В свою очередь `let` и `const` попадают именно в лексическое окружение блоков, а не только функций как `var`.
