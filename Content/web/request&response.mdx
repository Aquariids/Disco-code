...
Доменное имя здесь лишь удобно выглядящее название для человека, что бы было легче запомнить. На самом деле, за каждый доменным именем стоит `IP` адрес.
...
`HTTP` работает при помощи отправки сообщений (запросов) и ответов от сервера, от клиента к серверу и назад.
...
### HTTP respone
Данные в ответ на отправляются уже с помощью `HTTP respone`, тут  примерно такое же сообщение как и было в запросе.
...
Мы уже говорили, что это протоколы взаимодействия которе определяют каким образом данные перемещаются по интернету, без сильных подробностей узнаем конкретнее ддля чего они нужны.
...
Делается это прежде чем, ответ будет отослан. Каждый из этих пакетов идет по своему пути, так они приходят намного быстрее.После того, как ответ получен, `TCP` из этих пакетов пересоберет данные которые были запрошенны. 
### IP - Internet Protocol
Работа `IP` заключается в том, что бы отправлять и регулировать отправку по нужному адресу пакеты, которые создал `TCP`. Именно `IP` удостоверяется, что данные доставлены по нужному `ip` адресу
