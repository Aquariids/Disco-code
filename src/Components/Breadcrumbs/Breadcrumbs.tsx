        ...
        return (
            <div className={s.breadcrumbs}>
                <div className={s.breadcrumbs__content}>
                    ...
                    <Link href={path[3] != undefined ? `/content/${page}` : '/content'}><span className={s.breadcrumbs__link}>{theme}</span><span className={s.slash}> / </span></Link>
                    {map && map.map(item => {
                        return (
                            <>
                                ...
                                {path[3] === item.category && slug ? <Link href={item.href1}><span className={s.breadcrumbs__link}>{slug}</span></Link> : ''}
                            </>
                        );
                    })}
